<template>
  <div
    ...
    class="relative bg-primary inline-flex justify-between items-center px-4 py-2 text-sm font-base ring-1 ring-gray-400 rounded shadow-md"
    :class="width"
    type="button"
  >
    <span class="text-secondary text-sm font-medium"
      >{{ title }} {{ selectedOption.value }}</span
    >
    ...
    <transition name="fade" mode="out-in" class="bg-white ring-1 ring-gray-400">
      ...
      >
        <ul class="overflow-y-auto text-sm">
          ...
          >
            <div
              class="flex items-center px-1 py-2 rounded hover:bg-gray-100"
            ...
            >
              <input
                ...
                class="radio w-4 h-4 accent-[#ffcf25] bg-gray-100 border-gray-300 rounded"
              /><label
                :for="'radio-label-' + option + index"
                class="w-fit ml-3 py-1 px-3 text-[#5B5B5B] text-sm font-base rounded"
              >
                {{ option }}
              </label>
            </div>
...
const { options, width, title, background } = defineProps([
  "options",
  "width",
  "title",
  "background",
...
watch(selectedOption, (newValue) => {
  emit("get-filters", { period: newValue });
});
onMounted(() => {
